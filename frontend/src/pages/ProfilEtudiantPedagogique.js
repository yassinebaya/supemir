...
const ProfilEtudiantPedagogique = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const resPedago = await fetch('http://195.179.229.230:5000/api/pedagogique/me', config);
      if (resPedago.ok) {
        ...
        if (resEtudiant.ok) {
          ...
          if (access) {
            ...
            const resPres = await fetch(`http://195.179.229.230:5000/api/presences/etudiant/${id}`, config);
            if (resPres.ok) {
